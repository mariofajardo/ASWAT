...
FILES<-dir(path = 'Images/Sample_8_Nat_5_10/',full.names = T)[1:6]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #   display(img)
  num_agregates <- 5
  if(filei=='Images/Sample_8_Nat_5_10/DSC_6488.JPG'){
  cutting_area<-list(c(1000:2000),c(400:1500))
  } else {
    cutting_area<-list(c(1400:2300),c(400:1500))
  }
...
FILES<-dir(path = 'Images/Sample_8_Nat_5_10/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_8_Nat_5_10.RData')
num_agregates <- 5
