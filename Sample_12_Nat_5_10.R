...
FILES<-dir(path = 'Images/Sample_12_Nat_5_10/',full.names = T)[1:6]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #   display(img)
  num_agregates <- 5
  cutting_area<-list(c(1200:2400),c(450:1500))
...
FILES<-dir(path = 'Images/Sample_12_Nat_5_10/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_12_Nat_5_10.RData')
num_agregates <- 5
