...
FILES<-dir(path = 'Images/Sample_5_Nat_0_5/',full.names = T)[1:6]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #   display(img)
  num_agregates <- 5
  cutting_area<-list(c(900:2100),c(500:1500))
...
FILES<-dir(path = 'Images/Sample_5_Nat_0_5/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_5_Nat_0_5.RData')
num_agregates <- 5
