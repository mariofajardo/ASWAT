...
FILES<-dir(path = 'Images/Sample_15_Crop_5_10/',full.names = T)[1:8]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #display(img)
  num_agregates <- 5
  cutting_area<-list(c(720:2000),c(590:1600))
  a<-!img[cutting_area[[1]],cutting_area[[2]],1]>.65
...
#####now do all of them ####
###the pictures are better recognized with a threshold of .68 in the red band
ncluster <-4
setMKLthreads(n=2)
cl <- makeCluster(ncluster) 
registerDoSNOW(cl)

FILES<-dir(path = 'Images/Sample_15_Crop_5_10/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_15_Crop_5_10.RData')
num_agregates <- 5
