...
FILES<-dir(path = 'Images/Sample_11_Nat_5_10/',full.names = T)[120:125]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #   display(img)
  num_agregates <- 5
  if(filei =='Images/Sample_11_Nat_5_10/DSC_6925.JPG') {
  cutting_area<-list(c(1000:1900),c(400:1500))
  } else {
  cutting_area<-list(c(1700:2600),c(500:1500))
  }
...
FILES<-dir(path = 'Images/Sample_11_Nat_5_10/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_11_Nat_5_10.RData')
num_agregates <- 5
