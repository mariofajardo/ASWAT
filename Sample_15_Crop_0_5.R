...
FILES<-dir(path = 'Images/Sample_15_Crop_0_5/',full.names = T)[1:6]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #   display(img)
  num_agregates <- 5
  cutting_area<-list(c(1000:2200),c(450:1500))
...
FILES<-dir(path = 'Images/Sample_15_Crop_0_5/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_15_Crop_0_5.RData')
num_agregates <- 5
