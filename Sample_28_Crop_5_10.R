...
FILES<-dir(path = 'Images/Sample_28_Crop_5_10/',full.names = T)[1:6]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #display(img)
  num_agregates <- 5
  cutting_area<-list(c(800:1930),c(450:1600))
...
FILES<-dir(path = 'Images/Sample_28_Crop_5_10/',full.names = T)
...
areas[[1]]<-areas[[1]][c(2,1,3,5,4)];names(areas[[1]])<-c(1:5)

saveRDS(areas,file = 'RData/Sample_28_Crop_5_10.RData')
num_agregates <- 5
