...
FILES<-dir(path = 'Images/Sample_17_Crop_5_10/',full.names = T)[1:4]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #display(img)
  num_agregates <- 5
  cutting_area<-list(c(900:1900),c(550:1700))
...
FILES<-dir(path = 'Images/Sample_17_Crop_5_10/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_17_Crop_5_10.RData')
num_agregates <- 5
