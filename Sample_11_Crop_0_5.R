...
FILES<-dir(path = 'Images/Sample_11_Crop_0_5/',full.names = T)[1:6]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #   display(img)
  num_agregates <- 5
  cutting_area<-list(c(1200:2200),c(500:1400))
...
FILES<-dir(path = 'Images/Sample_11_Crop_0_5/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_11_Crop_0_5.RData')
num_agregates <- 5
