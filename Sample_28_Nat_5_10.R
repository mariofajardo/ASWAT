...
FILES<-dir(path = 'Images/Sample_28_Nat_5_10/',full.names = T)[1:6]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #   display(img)
  num_agregates <- 5
  cutting_area<-list(c(900:2000),c(400:1600))
...
FILES<-dir(path = 'Images/Sample_28_Nat_5_10/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_28_Nat_5_10.RData')
num_agregates <- 5
