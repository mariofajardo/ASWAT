...
#selecting the red band##
ncluster <-4
setMKLthreads(n=2)
cl <- makeCluster(ncluster) 
registerDoSNOW(cl)

FILES<-dir(path = 'Images/Sample_4_Crop_5_10/',full.names = T)[1:4]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #display(img)
  num_agregates <- 5
  cutting_area<-list(c(800:2100),c(590:1600))
  a<-!img[cutting_area[[1]],cutting_area[[2]],1]>.6
...
setwd('..')
#####now do all of them ####
###the first picture is better recognized with a threshold of .7 and 
### the others with a threshold of .6
ncluster <-4
setMKLthreads(n=2)
cl <- makeCluster(ncluster) 
registerDoSNOW(cl)

FILES<-dir(path = 'Images/Sample_4_Crop_5_10/',full.names = T)
...
areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  ...
  num_agregates <- 5
  cutting_area<-list(c(800:2100),c(590:1600))
  a<-!img[cutting_area[[1]],cutting_area[[2]],3]>.6
  #   display(a,method='raster')
  #Apply some filters for taking the '0' values inside de agregate#
  y <- closing(a, makeBrush(5, shape='disc'))
  #check#
...
FILES<-dir(path = 'Images/Sample_4_Crop_5_10/',full.names = T)
  ...
  area1<-foreach(filei=FILES[1],.packages = 'EBImage') %dopar%  {
    img<-readImage(filei) 
    # display(img)
    num_agregates <- 5
    cutting_area<-list(c(800:2100),c(590:1600))
    a<-!img[cutting_area[[1]],cutting_area[[2]],3]>.7
    #   display(a,method='raster')
    #Apply some filters for taking the '0' values inside de agregate#
    y <- closing(a, makeBrush(5, shape='disc'))
    #check#
...
saveRDS(areas,file = 'RData//Sample_4_Crop_5_10.RData')
num_agregates <- 5
observations<-c(seq(0,120,1),seq(140,360,20),seq(420,7200,600))
par(mfrow=c(num_agregates))
for (i in 1:num_agregates){
