...
FILES<-dir(path = 'Images/Sample_16_Crop_5_10/',full.names = T)[1:6]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #display(img)
  num_agregates <- 5
  cutting_area<-list(c(900:1930),c(450:1650))
...
FILES<-dir(path = 'Images/Sample_16_Crop_5_10/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_16_Crop_5_10.RData')
num_agregates <- 5
