...
FILES<-dir(path = 'Images/Sample_20_Nat_5_10/',full.names = T)[1:5]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #display(img)
  num_agregates <- 5
  cutting_area<-list(c(750:2000),c(550:1700))
...
FILES<-dir(path = 'Images/Sample_20_Nat_5_10/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_20_Nat_5_10.RData')
num_agregates <- 5
