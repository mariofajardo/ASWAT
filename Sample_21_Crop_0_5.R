...
FILES<-dir(path = 'Images/Sample_21_Crop_0_5/',full.names = T)[1:6]

areas<-foreach(filei=FILES,.packages = 'EBImage') %dopar%  {
  img<-readImage(filei) 
  #   display(img)
  num_agregates <- 5
  cutting_area<-list(c(1000:2200),c(600:1800))
...
FILES<-dir(path = 'Images/Sample_21_Crop_0_5/',full.names = T)
...
saveRDS(areas,file = 'RData/Sample_21_Crop_0_5.RData')
num_agregates <- 5
